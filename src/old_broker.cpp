...
void OldMqttBroker::subscribe(MqttSubscriber& subscriber, std::vector<std::string> topics) {
...
void OldMqttBroker::subscribe(MqttSubscriber& subscriber, std::vector<MqttSubscribe::Topic> topics) {
...
void OldMqttBroker::unsubscribe(MqttSubscriber& subscriber) {
    _subscriptions.removeSubscription(subscriber, _subscriptions._nodes);
}

void OldMqttBroker::unsubscribe(MqttSubscriber& subscriber, std::vector<std::string> topics) {
    _subscriptions.removeSubscription(subscriber, topics, _subscriptions._nodes);
}

void OldMqttBroker::publish(const std::string& topic, const std::string& payload) {
    std::vector<ubyte> realPayload(payload.begin(), payload.end());
    publish(topic, realPayload);
}

void OldMqttBroker::publish(const std::string& topic, const std::vector<ubyte>& payload) {
    TopicParts topParts;
    boost::split(topParts, topic, boost::is_any_of("/"));
    publish(topic, topParts, payload);
}

void OldMqttBroker::publish(const std::string& topic,
